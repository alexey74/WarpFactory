...
def ricci_tensor(input_tensor: np.ndarray[np.float64], inverse_tensor: np.ndarray[np.float64],
                   delta: np.ndarray[np.float64], dim: int = 4, order: int = 4, use_gpu: bool = False) -> np.ndarray[np.float64]:
    phi_flag: bool = False
    ...
    for i in range(dim):
        for j in range(i, dim):
            for k in range(dim):
                diff_1[i, j, k] = take_finite_diff_1dir(input_tensor[i, j], k, delta, phi_flag, order)
                        ...
                        for r in range(dim):
                            for d in range(dim):
                                ...
                                r_temp_1 += 1/2 * (1/2 * diff_1[a, r, i] * diff_1[b, d, j] + diff_1[i, r, a] * diff_1[j, d, b] -
                                                diff_1[i, r, a] * diff_1[j, b, d]) * inverse_tensor[a, b] * inverse_tensor[r, d]
                                # Third term
