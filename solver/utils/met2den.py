...
def met2den(input_tensor: np.ndarray[np.float64], delta: np.ndarray[np.float64] = np.array([1, 1, 1, 1]),
    ...
    ricci_t: np.ndarray[np.float64] = ricci_tensor(input_tensor, inverse_tensor, delta, order=order, use_gpu=use_gpu)

    ricci_s: np.float64 = ricci_scalar(inverse_tensor, ricci_t, use_gpu=use_gpu)

    return energy_density(einstein_tensor(input_tensor, ricci_t, ricci_s, use_gpu=use_gpu), inverse_tensor, use_gpu=use_gpu)
