...
def take_finite_diff_1dir(tensor: np.ndarray[np.float64], k: int, delta: np.ndarray[np.float64],
                          phi_flag: bool = False, order: int = 4) -> np.ndarray[np.float64]:
    size: tuple[int, ...] = tensor.shape

    big_b: np.ndarray[np.float64] = np.zeros(size)

    if order == 4 and size[k] >= 5:
        if k == 0:
...
def take_finite_diff_2dirs(tensor: np.ndarray[np.float64], k1: int, k2: int,
                           delta: np.ndarray[np.float64], phi_flag: bool = False, order: int = 4) -> np.ndarray[np.float64]:
...
def _take_finite_diff_2dirs_second_order(tensor: np.ndarray[np.float64], k1: int, k2: int,
                                         delta: np.ndarray[np.float64]) -> np.ndarray[np.float64]:
    size: tuple[int, ...] = tensor.shape
    ...
    if size[k1] >= 3 and size[k2] >= 3:
        if k1 == k2:
            if k1 == 0:
                big_b[1:-2, :, :, :] = (tensor[2:, :, :, :] - 2 * tensor[1:-2, :, :, :] + tensor[:-3, :, :, :]) / (delta[k1] ** 2)
                big_b[0, :, :, :] = big_b[1, :, :, :]
                big_b[-1, :, :, :] = big_b[-2, :, :, :]
            elif k1 == 1:
            ...
            if k_s == 0:
                if k_l == 1:  # partial t / partial x
                    big_b[x0, y0, :, :] = 1 / (2 ** 2 * delta[k_l] * delta[k_s]) * (
                            tensor[x_1, y_1, :, :] - tensor[x_1, y1, :, :] - tensor[x1, y_1, :, :] + tensor[x1, y1, :, :])
...
def _take_finite_diff_2dirs_fourth_order(tensor: np.ndarray[np.float64], k1: int, k2: int,
                           delta: np.ndarray[np.float64], phi_flag: bool) -> np.ndarray[np.float64]:
    size: tuple[int, ...] = tensor.shape
