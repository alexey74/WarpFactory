...
from solver import verify_tensor, met2den, Energy
...
def get_energy_tensor(metric_val: Metric, diff_order: int = 4, use_gpu: bool = False) -> Energy:
    ...
    if use_gpu:
        ...
        energy_tensor = met2den(metric_val.tensor, metric_val.scaling, order=diff_order)
    else:
        energy_tensor = met2den(metric_val.tensor, metric_val.scaling, order=diff_order)
