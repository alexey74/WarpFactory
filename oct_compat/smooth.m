...
function yy = smooth (varargin)
  ...
  if span > 0 && span < 1 %% fraction
    span = ceil(length(y) * span);
  endif

  ## Perform smoothing

  if (span > length (y))
    error (sprintf("smooth: span cannot be greater than ''length (y)'': %f > %f", span, length(y)))
  endif
  yy = [];
  switch method
    ## --- Moving average
    case 'moving'
      for i=1:length (y)
        if (mod (span,2) == 0)
          # error (sprintf('smooth: span must be odd: %f', span))
          span = max([span-1, 1]);
        endif
        if (i <= (span-1)/2)
